...
Blueprint terpisah (pola sama dgn admin_afk.py). Mengubah teks DM yang dikirim ke
member beberapa hari sebelum langganannya habis. Cog `cogs/sub_followup.py` membaca
teks lewat utils.sub_followup_text, jadi perubahan langsung dipakai pengiriman
berikutnya.
...
_SAMPLE = {"store": "Cellyn Store", "item": "Spotify Premium 1 Bulan", "waktu": "dalam 3 hari lagi"}
...
def save_sub_followup_route():
    ...
    if kind not in sftext.SUB_FOLLOWUP_SPECS:
    ...
    sftext.save_text(kind, text=text)
...
def reset_sub_followup_route():
    ...
    if kind not in sftext.SUB_FOLLOWUP_SPECS:
        return jsonify({"ok": False, "error": "Jenis teks tidak dikenal."}), 400
    sftext.save_text(kind, text="")
    return jsonify({"ok": True, "text": sftext.load_text(kind)})
...
def page_sub_followup():
    ...
    sections = []
    for kind, spec in sftext.SUB_FOLLOWUP_SPECS.items():
        sections.append({
            "kind": kind,
            "label": spec["label"],
            "text": sftext.load_text(kind),
...
<div class="page-header">
  <div class="page-title">DM Perpanjangan <small>Pengingat langganan hampir habis</small></div>
</div>
...
function saveSec(i){
  var s = SECTIONS[i];
  fetch('/subfollowup-editor/save',{method:'POST',headers:{'Content-Type':'application/json'},
...
function resetSec(i){
  ...
  fetch('/subfollowup-editor/reset',{method:'POST',headers:{'Content-Type':'application/json'},
