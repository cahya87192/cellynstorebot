...
Blueprint terpisah (pola sama dgn admin_afk.py). Mengubah pesan saat admin
menjalankan !done (selesai) / !cancel (batal) pada tiket order layanan "lainnya".
Cog `cogs/orders.py` membaca teks lewat utils.order_text, jadi perubahan langsung
dipakai berikutnya.
...
def save_order_route():
    ...
    if kind not in otext.ORDER_SPECS:
    ...
    otext.save_text(kind, text=text)
...
def reset_order_route():
    ...
    if kind not in otext.ORDER_SPECS:
        return jsonify({"ok": False, "error": "Jenis teks tidak dikenal."}), 400
    otext.save_text(kind, text="")
    return jsonify({"ok": True, "text": otext.load_text(kind)})
...
def page_order():
    ...
    sections = []
    for kind, spec in otext.ORDER_SPECS.items():
        sections.append({
            "kind": kind,
            "label": spec["label"],
            "text": otext.load_text(kind),
...
<div class="page-header">
  <div class="page-title">Pesan Order <small>Notifikasi selesai (!done) &amp; batal (!cancel)</small></div>
</div>
...
function saveSec(i){
  var s = SECTIONS[i];
  fetch('/order-editor/save',{method:'POST',headers:{'Content-Type':'application/json'},
...
function resetSec(i){
  ...
  fetch('/order-editor/reset',{method:'POST',headers:{'Content-Type':'application/json'},
