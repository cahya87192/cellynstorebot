...
Blueprint terpisah (pola sama dgn admin_vilog.py). Mengubah prosa panel katalog
Topup Robux via Gamepass (judul/deskripsi/cara order/catatan/footer) dan pesan
selesai. Cog `cogs/gp.py` membaca teks lewat utils.gp_text, jadi perubahan dipakai
saat katalog di-refresh / tiket diproses berikutnya.
...
_SAMPLE = {"store": "Cellyn Store", "min": "300"}
...
def save_gp_route():
    ...
    if kind not in gptext.GP_SPECS:
    ...
    gptext.save_text(kind, text=text)
...
def reset_gp_route():
    ...
    if kind not in gptext.GP_SPECS:
        return jsonify({"ok": False, "error": "Jenis teks tidak dikenal."}), 400
    gptext.save_text(kind, text="")
    return jsonify({"ok": True, "text": gptext.load_text(kind)})
...
def page_gp():
    ...
    sections = []
    for kind, spec in gptext.GP_SPECS.items():
        sections.append({
            "kind": kind,
            "label": spec["label"],
            "text": gptext.load_text(kind),
...
<div class="page-header">
  <div class="page-title">Katalog GP <small>Topup Robux via Gamepass — teks panel &amp; tiket</small></div>
</div>
...
function saveSec(i){
  var s = SECTIONS[i];
  fetch('/gp-editor/save',{method:'POST',headers:{'Content-Type':'application/json'},
...
function resetSec(i){
  ...
  fetch('/gp-editor/reset',{method:'POST',headers:{'Content-Type':'application/json'},
