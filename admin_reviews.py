...
Blueprint terpisah (pola sama dgn admin_afk.py). Mengubah prosa pesan rating:
prompt rating, struk/invoice, pemberitahuan kedaluwarsa, pengingat, judul ulasan,
dan ucapan terima kasih. Struktur embed (field, tombol bintang, timestamp) tetap
dikelola cog. Cog `cogs/reviews.py` membaca teks lewat utils.review_text.
...
def save_review_route():
    ...
    if kind not in rtext.REVIEW_SPECS:
    ...
    rtext.save_text(kind, text=text)
...
def reset_review_route():
    ...
    if kind not in rtext.REVIEW_SPECS:
        return jsonify({"ok": False, "error": "Jenis teks tidak dikenal."}), 400
    rtext.save_text(kind, text="")
    return jsonify({"ok": True, "text": rtext.load_text(kind)})
...
def page_review():
    ...
    sections = []
    for kind, spec in rtext.REVIEW_SPECS.items():
        sections.append({
            "kind": kind,
            "label": spec["label"],
            "text": rtext.load_text(kind),
...
<div class="page-header">
  <div class="page-title">Pesan Rating &amp; Ulasan <small>Prompt, struk, pengingat, kedaluwarsa &amp; ucapan terima kasih</small></div>
</div>
...
function saveSec(i){
  var s = SECTIONS[i];
  fetch('/review-editor/save',{method:'POST',headers:{'Content-Type':'application/json'},
...
function resetSec(i){
  ...
  fetch('/review-editor/reset',{method:'POST',headers:{'Content-Type':'application/json'},
