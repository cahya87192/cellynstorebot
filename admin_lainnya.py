...
LAYANAN (judul/deskripsi/footer) dan balasan auto-reply (judul/footer). Cog
`cogs/lainnya.py` membaca teks lewat utils.lainnya_text, jadi perubahan dipakai
saat katalog di-refresh / auto-reply dikirim berikutnya.
...
def save_lainnya_route():
    ...
    if kind not in latext.LAINNYA_SPECS:
    ...
    latext.save_text(kind, text=text)
...
def reset_lainnya_route():
    ...
    if kind not in latext.LAINNYA_SPECS:
        return jsonify({"ok": False, "error": "Jenis teks tidak dikenal."}), 400
    latext.save_text(kind, text="")
    return jsonify({"ok": True, "text": latext.load_text(kind)})
...
def page_lainnya_text():
    ...
    sections = []
    for kind, spec in latext.LAINNYA_SPECS.items():
        sections.append({
            "kind": kind,
            "label": spec["label"],
            "text": latext.load_text(kind),
...
<div class="page-header">
  <div class="page-title">Katalog Lainnya <small>Layanan Lainnya — teks panel &amp; auto-reply</small></div>
</div>
...
function saveSec(i){
  var s = SECTIONS[i];
  fetch('/lainnya-editor/save',{method:'POST',headers:{'Content-Type':'application/json'},
...
function resetSec(i){
  ...
  fetch('/lainnya-editor/reset',{method:'POST',headers:{'Content-Type':'application/json'},
