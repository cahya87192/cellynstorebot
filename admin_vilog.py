...
Blueprint terpisah (pola sama dgn admin_afk.py). Mengubah prosa panel katalog
Topup Robux via Login (judul/deskripsi/catatan/footer), pesan selesai, dan judul
pembatalan. Cog `cogs/vilog.py` membaca teks lewat utils.vilog_text, jadi
perubahan dipakai saat katalog di-refresh / tiket diproses berikutnya.
...
_SAMPLE = {"store": "Cellyn Store", "step": "500", "max": "10000"}
...
def save_vilog_route():
    ...
    if kind not in vtext.VILOG_SPECS:
    ...
    vtext.save_text(kind, text=text)
...
def reset_vilog_route():
    ...
    if kind not in vtext.VILOG_SPECS:
        return jsonify({"ok": False, "error": "Jenis teks tidak dikenal."}), 400
    vtext.save_text(kind, text="")
    return jsonify({"ok": True, "text": vtext.load_text(kind)})
...
def page_vilog():
    ...
    sections = []
    for kind, spec in vtext.VILOG_SPECS.items():
        sections.append({
            "kind": kind,
            "label": spec["label"],
            "text": vtext.load_text(kind),
...
<div class="page-header">
  <div class="page-title">Katalog Vilog <small>Topup Robux via Login — teks panel &amp; tiket</small></div>
</div>
...
function saveSec(i){
  var s = SECTIONS[i];
  fetch('/vilog-editor/save',{method:'POST',headers:{'Content-Type':'application/json'},
...
function resetSec(i){
  ...
  fetch('/vilog-editor/reset',{method:'POST',headers:{'Content-Type':'application/json'},
