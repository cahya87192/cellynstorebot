"""admin_profile_text.py - Editor teks embed Badge & fallback Profil.

Blueprint terpisah (pola sama dgn admin_afk.py). Mengubah prosa kecil di embed
/badges (cogs/achievements.py) & fallback /profil (cogs/profile.py). Cog membaca
teks lewat utils.badge_profile_text, jadi perubahan langsung dipakai berikutnya.

  - /profiltext-editor          : form per jenis teks + pratinjau langsung
  - /profiltext-editor/save     : simpan teks (POST JSON {kind,text})
  - /profiltext-editor/reset    : kembalikan satu jenis ke default (POST JSON {kind})
"""
import json
...
profile_text_bp = Blueprint("profile_text_bp", __name__)

# Nilai contoh per placeholder untuk pratinjau langsung.
_SAMPLE = {"name": "Andi", "store": "Cellyn Store"}
...
@profile_text_bp.route("/profiltext-editor/save", methods=["POST"])
def save_profile_text_route():
    g = _guard()
    if g:
        return g
    payload = request.get_json(force=True, silent=True) or {}
    kind = payload.get("kind")
    if kind not in bptext.BADGE_PROFILE_SPECS:
    ...
    bptext.save_text(kind, text=text)
    return jsonify({"ok": True})


@profile_text_bp.route("/profiltext-editor/reset", methods=["POST"])
def reset_profile_text_route():
    g = _guard()
    if g:
        return g
    payload = request.get_json(force=True, silent=True) or {}
    ...
    if kind not in bptext.BADGE_PROFILE_SPECS:
        return jsonify({"ok": False, "error": "Jenis teks tidak dikenal."}), 400
    bptext.save_text(kind, text="")
    return jsonify({"ok": True, "text": bptext.load_text(kind)})


@profile_text_bp.route("/profiltext-editor")
def page_profile_text():
    g = _guard()
    if g:
        return g
    ...
    sections = []
    for kind, spec in bptext.BADGE_PROFILE_SPECS.items():
        sections.append({
            "kind": kind,
            "label": spec["label"],
            "text": bptext.load_text(kind),
...
<div class="page-header">
  <div class="page-title">Teks Badge &amp; Profil <small>Embed /badges &amp; fallback /profil</small></div>
</div>
<div class="card"><div class="card-body">
  <div class="note" style="margin-bottom:1rem;">
    Teks prosa kecil di embed <code>/badges</code> &amp; embed fallback <code>/profil</code>
    (dipakai bila render kartu gambar gagal). Perubahan langsung dipakai berikutnya.
...
function render(tpl, sample){
  ...
  return out.replace(/\\*\\*([^*]+)\\*\\*/g, "<b>$1</b>").replace(/_([^_]+)_/g, "<i>$1</i>").replace(/\\n/g, "<br>");
}
...
function saveSec(i){
  var s = SECTIONS[i];
  fetch('/profiltext-editor/save',{method:'POST',headers:{'Content-Type':'application/json'},
...
function resetSec(i){
  ...
  fetch('/profiltext-editor/reset',{method:'POST',headers:{'Content-Type':'application/json'},
