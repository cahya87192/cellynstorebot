...
ROBUX STORE (judul/deskripsi/footer). Cog `cogs/robux.py` membaca teks lewat
utils.robux_text, jadi perubahan dipakai saat katalog di-refresh berikutnya.
...
def save_robux_route():
    ...
    if kind not in rbtext.ROBUX_SPECS:
    ...
    rbtext.save_text(kind, text=text)
...
def reset_robux_route():
    ...
    if kind not in rbtext.ROBUX_SPECS:
        return jsonify({"ok": False, "error": "Jenis teks tidak dikenal."}), 400
    rbtext.save_text(kind, text="")
    return jsonify({"ok": True, "text": rbtext.load_text(kind)})
...
def page_robux():
    ...
    sections = []
    for kind, spec in rbtext.ROBUX_SPECS.items():
        sections.append({
            "kind": kind,
            "label": spec["label"],
            "text": rbtext.load_text(kind),
...
<div class="page-header">
  <div class="page-title">Katalog Robux <small>Robux Store — teks panel katalog</small></div>
</div>
...
function saveSec(i){
  var s = SECTIONS[i];
  fetch('/robux-editor/save',{method:'POST',headers:{'Content-Type':'application/json'},
...
function resetSec(i){
  ...
  fetch('/robux-editor/reset',{method:'POST',headers:{'Content-Type':'application/json'},
