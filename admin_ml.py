...
Topup Diamond (judul/deskripsi/footer), pesan selesai, dan judul pembatalan. Cog
`cogs/ml.py` membaca teks lewat utils.ml_text, jadi perubahan dipakai saat katalog
di-refresh / tiket diproses berikutnya.
...
def save_ml_route():
    ...
    if kind not in mltext.ML_SPECS:
    ...
    mltext.save_text(kind, text=text)
...
def reset_ml_route():
    ...
    if kind not in mltext.ML_SPECS:
        return jsonify({"ok": False, "error": "Jenis teks tidak dikenal."}), 400
    mltext.save_text(kind, text="")
    return jsonify({"ok": True, "text": mltext.load_text(kind)})
...
def page_ml():
    ...
    sections = []
    for kind, spec in mltext.ML_SPECS.items():
        sections.append({
            "kind": kind,
            "label": spec["label"],
            "text": mltext.load_text(kind),
...
<div class="page-header">
  <div class="page-title">Katalog ML <small>Topup Diamond — teks panel &amp; tiket</small></div>
</div>
...
function saveSec(i){
  var s = SECTIONS[i];
  fetch('/ml-editor/save',{method:'POST',headers:{'Content-Type':'application/json'},
...
function resetSec(i){
  ...
  fetch('/ml-editor/reset',{method:'POST',headers:{'Content-Type':'application/json'},
