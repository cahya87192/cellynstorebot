...
Blueprint terpisah (pola sama dgn admin_afk.py). Mengubah prosa di sekitar FAQ:
judul/deskripsi/footer embed FAQ, footer balasan Auto-CS, & pesan /saran. Knowledge
base (pertanyaan/jawaban) tetap diedit di halaman "Editor FAQ" yang lama. Cog
`cogs/faq.py` membaca teks lewat utils.faq_text, jadi perubahan dipakai berikutnya
(embed FAQ di-update saat admin menjalankan `!faqrefresh`).
...
_SAMPLE = {"store": "Cellyn Store", "question": "Berapa lama proses topup?"}
...
def save_faq_text_route():
    ...
    if kind not in fqtext.FAQ_TEXT_SPECS:
    ...
    fqtext.save_text(kind, text=text)
...
def reset_faq_text_route():
    ...
    if kind not in fqtext.FAQ_TEXT_SPECS:
        return jsonify({"ok": False, "error": "Jenis teks tidak dikenal."}), 400
    fqtext.save_text(kind, text="")
    return jsonify({"ok": True, "text": fqtext.load_text(kind)})
...
def page_faq_text():
    ...
    sections = []
    for kind, spec in fqtext.FAQ_TEXT_SPECS.items():
        sections.append({
            "kind": kind,
            "label": spec["label"],
            "text": fqtext.load_text(kind),
...
<div class="page-header">
  <div class="page-title">Teks FAQ <small>Embed FAQ, Auto-CS &amp; pesan /saran</small></div>
</div>
...
function saveSec(i){
  var s = SECTIONS[i];
  fetch('/faq-text-editor/save',{method:'POST',headers:{'Content-Type':'application/json'},
...
function resetSec(i){
  ...
  fetch('/faq-text-editor/reset',{method:'POST',headers:{'Content-Type':'application/json'},
